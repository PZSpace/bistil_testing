...
class ModelArguments:
    ...
    head_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to model head."}
    )
...
class DataTrainingArguments:
    ...
    compute_metrics: bool = field(
        default=True, metadata={"help": "Whether to compute evaluation metrics."}
    )
...
def main():
    ...
    if sft_args.task_ft is not None:
        #logger.info(f'Applying sparse language model {sft_args.task_ft}.')
        #with torch.no_grad():
        #    task_ft = torch.load(sft_args.task_ft)
        #    for n, p in task_ft.items():
        #        try:
        #            param = model.get_parameter(n)
        #        except:
        #            logger.info(f'Model does not contain parameter {n}.')
        #            continue
        #        if not n.startswith(model.base_model_prefix):
        #            logger.info(f'{n}: {param}')
        #            param.zero_()
        #        p = p.to(param.device)
        #        num_non_zeros = (param != 0.0).float()
        #        num_non_zeros.add_((p != 0.0).float())
        #        num_non_zeros = torch.maximum(num_non_zeros, torch.ones_like(num_non_zeros))
        #        param.copy_((param + p) / num_non_zeros)
    ...
    if sft_args.lang_ft is not None:
        ...
        #    )
        #logger.info(f'Applying sparse language model {sft_args.lang_ft}.')
        #with torch.no_grad():
        #    lang_ft = torch.load(sft_args.lang_ft)
        #    for n, p in lang_ft.items():
        #        try:
        #            param = model.get_parameter(n)
        #        except:
        #            logger.info(f'Model does not contain parameter {n}.')
    ...
    trainer = trainer_cls(
        ...
        compute_metrics=compute_metrics if data_args.compute_metrics else None,
